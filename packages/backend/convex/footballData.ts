...
export const syncAll = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    let url = `${API_BASE}/competitions/${COMPETITION_CODE}/matches`;
    const params: string[] = [];
    if (args.dateFrom) params.push(`dateFrom=${args.dateFrom}`);
    if (args.dateTo) params.push(`dateTo=${args.dateTo}`);
    ...
    for (const match of data.matches) {
      ...
      const homeTeamId = await ctx.runMutation(
        internal.matches.upsertTeam,
        {
      ...
      const awayTeamId = await ctx.runMutation(
        internal.matches.upsertTeam,
        {
      ...
      const status = normalizeStatus(match.status) as
        | "TIMED" | "SCHEDULED" | "LIVE" | "IN_PLAY"
        | "PAUSED" | "FINISHED" | "POSTPONED" | "CANCELLED";
    ...
    console.log(`Synced ${synced} matches, computed points for ${pointsComputed}`);
