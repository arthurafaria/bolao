...
export default function ManageLeaguePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const leagueId = id as Id<"leagues">;

  const league = useQuery(api.leagues.getById, { leagueId });
  const currentUser = useQuery(api.auth.getCurrentUser);
  const pendingRequests = useQuery(api.leagues.getPendingRequests, { leagueId });
  ...
  if (league.ownerId !== currentUser._id) {
    return <div className="text-center py-12 text-muted-foreground">Acesso negado</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold">Gerenciar liga</h1>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {pendingRequests.map((req) => (
              <div key={req._id} className="flex items-center justify-between rounded-lg border border-border p-3">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Membros ({league.memberCount})</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {ranking?.map((member) => (
            <div key={member._id} className="flex items-center justify-between rounded-lg border border-border p-3">
              <div>
                <span className="text-sm font-medium">
                  {member.userId === currentUser._id ? "Você (admin)" : `Jogador ${member.userId.slice(-4)}`}
                </span>
